...
impl AuthenticationTokenHeader {
    pub async fn validate(
        ...
        if self.0.is_empty() {
            return Err(AuthenticationError::InvalidToken { token: self.0 });
        }
        ...
        match auth_response {
            ...
            NautilusAuthResponse::Info {
                ...
                let team_id = team.id;
                let _: () = redis_con.set(&redis_key_team, team_id).await?;
                let _: () = redis_con.expire(&redis_key_team, 15 * 60).await?;
                let _: () = redis_con
                    .set(&redis_key_challenge, challenge.shortname.clone())
                    .await?;
                let _: () = redis_con.expire(&redis_key_challenge, 15 * 60).await?;
