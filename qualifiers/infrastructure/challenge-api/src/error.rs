...
pub enum ApiError {
    ...
    #[display("ChallengeNotFound: challenge: {challenge_id}")]
    ChallengeNotFound {
        challenge_id: ChallengeId,
    },
    #[display("ChallengeClosed: challenge: {challenge_id}")]
    ChallengeClosed {
        challenge_id: ChallengeId,
    },
    #[display("ChallengeNotReleased: challenge: {challenge_id}")]
    ChallengeNotReleased {
        challenge_id: ChallengeId,
    },
    #[display("ExploitNotFound: exploit: {exploit_id}")]
    ExploitNotFound {
        exploit_id: Uuid,
    },
    #[display("ExploitForbidden: team: {team_id}, exploit: {exploit_id}")]
    ExploitForbidden {
        team_id: TeamId,
        exploit_id: Uuid,
    },
    ...
    #[display("ExploitAlreadyQueued: team: {team_id}, challenge: {challenge_id}")]
...
impl From<deadpool_lapin::lapin::Error> for ApiError {
...
impl actix_web::error::ResponseError for ApiError {
    fn error_response(&self) -> HttpResponse {
        let error_body = match self {
            ...
            Self::FileIO { .. } => ApiErrorMessage::new("Internal server error".to_string()),
            Self::MessageQueue { .. } => ApiErrorMessage::new("Internal server error".to_string()),

            Self::RateLimit { limit } => ApiErrorMessage::new(format!(
                "Rate limit exceeded: can only submit once every {limit} seconds"
            )),

            // TODO (P2): map to specific errors (missing Dockerfile vs max size exceeded)
    ...
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            Self::MessageQueue { .. } => StatusCode::INTERNAL_SERVER_ERROR,
            Self::FileIO { .. } => StatusCode::INTERNAL_SERVER_ERROR,
